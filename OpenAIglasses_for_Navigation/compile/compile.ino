...
// ===== WiFi / Server =====
const char* WIFI_SSID   = "VickyZhai";
const char* WIFI_PASS   = "dream518998";
...
framesize_t g_frame_size = FRAMESIZE_SVGA;   // 640x480 - 降低分辨率减轻栈压力
#define JPEG_QUALITY  12                    // 适当压缩，减小帧体积（5-40，越小越清晰）
#define FB_COUNT      2
volatile int g_target_fps = 10;             // 目标FPS降到10，减轻发送压力
...
#define I2S_SPK_LRCK 8  //D9
#define I2S_SPK_DIN  9  //D10
...
void taskCamSend(void*) {
  ...
  int consecutive_fail_count = 0;  // 连续失败计数
  const int MAX_CONSECUTIVE_FAILS = 10;  // 【优化】提高到10次才断开，增加容错
  unsigned long last_slow_send = 0;  // 上次慢发送时间，用于自适应降速
  int adaptive_skip = 0;  // 自适应跳帧计数
  
  for(;;){
    fb_ptr_t fb = nullptr;
    if (xQueueReceive(qFrames, &fb, pdMS_TO_TICKS(100)) == pdPASS) {
      if (fb && cam_ws_ready) {
        // 【优化】自适应跳帧：如果最近有慢发送，跳过一些帧减轻压力
        if (adaptive_skip > 0) {
          adaptive_skip--;
          esp_camera_fb_return(fb);
          frame_dropped_count++;  // 统计为主动丢帧
          continue;
        }
        
        // 发送节流：若设置了目标FPS，则按周期发
        if (g_target_fps > 0) {
          const int period_ms = 1000 / g_target_fps;
          TickType_t now = xTaskGetTickCount();
          int elapsed = (now - lastTick) * portTICK_PERIOD_MS;
          // 【优化】如果距上次发送已经很久，不再等待（避免积压后的额外延迟）
          if (elapsed < period_ms && elapsed >= 0) {
            // 【优化】缩短最大等待时间，避免长时间阻塞
            int wait_ms = min(period_ms - elapsed, 30);
        ...
        if (ok) {
          ...
          consecutive_fail_count = 0;  // 成功后重置失败计数
          
          // 【优化】自适应降速：如果发送耗时过长，启动跳帧
          if (send_time > 150) {
            // 根据发送耗时动态计算跳帧数
            if (send_time > 400) {
              adaptive_skip = 3;  // 严重拥塞，跳3帧
            } else if (send_time > 250) {
              adaptive_skip = 2;  // 中度拥塞，跳2帧
            } else {
              adaptive_skip = 1;  // 轻度拥塞，跳1帧
            }
            last_slow_send = millis();
            Serial.printf("[CAM-SEND] slow send %lu ms, skip next %d frames\n", send_time, adaptive_skip);
          }
        } else {
          ws_send_fail_count++;
          consecutive_fail_count++;
          Serial.printf("[CAM-SEND] send failed (%d/%d)\n", consecutive_fail_count, MAX_CONSECUTIVE_FAILS);
          esp_camera_fb_return(fb);
          
          // 连续失败超过阈值才断开连接
          if (consecutive_fail_count >= MAX_CONSECUTIVE_FAILS) {
            Serial.println("[CAM-SEND] Too many failures, closing connection...");
            wsCam.close(); 
            cam_ws_ready = false;
        ...
        unsigned long now = millis();
        if (now - last_log > 5000) {
          int8_t rssi = WiFi.RSSI();  // 添加WiFi信号强度监控
          float actual_fps = (float)frame_sent_count / ((now - last_stats_time) / 1000.0f);
          Serial.printf("[CAM-SEND] sent=%lu, dropped=%lu, fail=%lu, fps=%.1f, RSSI=%d dBm\n", 
                        frame_sent_count, frame_dropped_count, ws_send_fail_count, actual_fps, rssi);
          last_log = now;
          // 重置统计用于下一周期
          frame_sent_count = 0;
...
static inline void mono16_to_stereo32_msb(const int16_t* in, size_t nSamp, int32_t* outLR, float gain = 0.2f) {
  // 【降低音量】从0.7降到0.4（范围0.0-1.0，越小越安静）
...
void setup() {
  ...
  // --- 启用麦克风和扬声器 ---
  init_i2s_in();          // 开启 PDM Mic 采集
  init_i2s_out();         // 【修复】启用扬声器输出，避免空指针崩溃

  qFrames = xQueueCreate(4, sizeof(fb_ptr_t));  // 【优化】增加到4个缓冲，应对网络波动
  qAudio  = xQueueCreate(AUDIO_QUEUE_DEPTH, sizeof(AudioChunk));  // 上行音频队列
  qTTS    = xQueueCreate(TTS_QUEUE_DEPTH, sizeof(TTSChunk));   // 扬声器相关队列保持关闭

  xTaskCreatePinnedToCore(taskCamCapture, "cam_cap", 12288, NULL, 4, NULL, 1);
  xTaskCreatePinnedToCore(taskCamSend,    "cam_snd", 10240, NULL, 3, NULL, 1);
  xTaskCreatePinnedToCore(taskMicCapture, "mic_cap",   4096, NULL, 2, NULL, 0);  // 采集麦克风
  xTaskCreatePinnedToCore(taskMicUpload,  "mic_upl",   4096, NULL, 2, NULL, 1);  // 发送到 /ws_audio
  xTaskCreatePinnedToCore(taskImuLoop,    "imu_loop",  4096, NULL, 2, NULL, 0);  // 暂时关闭 IMU，避免竞争带宽
  ...
  wsAud.onEvent([](WebsocketsEvent ev, String){
    ...
    if (ev == WebsocketsEvent::ConnectionClosed)  { 
      aud_ws_ready = false; 
      run_audio_stream = false;  // 【关键修复】断开时重置，避免重连后跳过START
      Serial.println("[WS-AUD] closed, audio stream stopped"); 
      stopStreamWav();
    }
  });
  ...
  wsAud.onMessage([](WebsocketsMessage msg){
    if (msg.isText()){
      ...
      Serial.printf("[WS-AUD] Received: %s\n", s.c_str());
      if (s == "RESTART"){
        Serial.println("[WS-AUD] Restarting ASR session...");
        run_audio_stream = false; 
        xQueueReset(qAudio); 
        delay(50);
        wsAud.send("START"); 
        run_audio_stream = true;
        Serial.println("[WS-AUD] ASR session restarted");
      }
    }
  });
}
...
// 重连退避变量
void loop() {
  static int cam_retry_count = 0;
  static int aud_retry_count = 0;
  static unsigned long last_wifi_check = 0;
  static unsigned long last_cam_ping = 0;  // 【新增】心跳时间
  ...
  if (!wsCam.available()) {
    // 指数退避：1s, 2s, 4s, 最大8s
    int backoff_ms = min(1000 << cam_retry_count, 8000);
    
    if (wsCam.connect(SERVER_HOST, SERVER_PORT, CAM_WS_PATH)) {
      Serial.println("[WS-CAM] connected");
      cam_retry_count = 0;  // 成功后重置
    } else { 
      cam_retry_count++;
      Serial.printf("[WS-CAM] retry in %d ms (attempt %d)\n", backoff_ms, cam_retry_count);
      delay(backoff_ms); 
    }
  }

  // 语音 WebSocket：只上传麦克风音频，不拉取 /stream.wav 播放
  if (!wsAud.available()) {
    int backoff_ms = min(2000 << aud_retry_count, 16000);
    
    if (wsAud.connect(SERVER_HOST, SERVER_PORT, AUD_WS_PATH)) {
      Serial.println("[WS-AUD] connected");
      aud_retry_count = 0;
      
      // 【改进】等待连接稳定后再发送START，并重试确保发送成功
      delay(100);  // 等待连接稳定
      
      bool start_sent = false;
      for (int i = 0; i < 3 && !start_sent; i++) {  // 最多重试3次
        if (wsAud.available()) {
          start_sent = wsAud.send("START");
          if (start_sent) {
            Serial.println("[WS-AUD] START sent successfully");
          } else {
            Serial.printf("[WS-AUD] START send failed, retry %d\n", i+1);
            delay(50);
          }
        } else {
          Serial.println("[WS-AUD] Connection lost before START");
          break;
        }
      }
      
      if (start_sent) {
        run_audio_stream = true;   // 只有START成功才开始音频流
        startStreamWav();          // 【启用】启动HTTP音频播放任务，拉取/stream.wav
      }
    } else {
      aud_retry_count++;
      Serial.printf("[WS-AUD] retry in %d ms (attempt %d)\n", backoff_ms, aud_retry_count);
      delay(backoff_ms);
    }
  }
  ...
  // 【新增】定期发送ping保活，防止服务器/NAT超时
  unsigned long now = millis();
  if (cam_ws_ready && (now - last_cam_ping) > 15000) {  // 每15秒ping一次
    wsCam.ping();
    last_cam_ping = now;
  }
